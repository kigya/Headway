 ...
 */
internal fun Route.usersRouting(usersService: UsersServiceContract) {
    getUserById(usersService)
    createUser(usersService)
    upsertGoogleUser(usersService)
}
 ...
 * @param usersService The service for handling user data operations.
 */
private fun Route.getUserById(usersService: UsersServiceContract) {
    get("/user") {
 ...
 * @param usersService The service for handling user data operations.
 */
private fun Route.createUser(usersService: UsersServiceContract) {
...
private fun Route.upsertGoogleUser(usersService: UsersServiceContract) {
    post("/user/google/upsert") {
        val request = try {
            call.receive<UpsertGoogleUserRequestDto>()
        } catch (e: Exception) {
            call.respondBadRequest(e)
            return@post
        }
        ...
        try {
            val user = usersService.upsertGoogleUser(
                googleId = request.googleId,
                email = request.email,
                name = request.name,
                avatarUrl = request.avatarUrl,
            ...
            call.application.environment.log.error("Failed to upsert user", e)
            call.respondServerError(e)
        }
    }
}
