...
read_feature_resolution_field() {
    local repo_root="$1"
    local field="$2"
    local file="$repo_root/.ai/specify/init-options.json"
    if [[ ! -f "$file" ]]; then
        return 1
    fi
    if has_jq; then
...
check_feature_branch() {
    ...
    echo "Or configure .ai/specify/init-options.json → feature_resolution (validate_git_branch, extra_branch_regex), use SPECIFY_FEATURE, or fixed_specs_subdir for FEATURE_DIR resolution." >&2
    return 1
}

get_feature_dir() { echo "$1/.ai/specs/$2"; }
...
find_feature_dir_by_prefix() {
    ...
    # Search for directories in .ai/specs/ that start with this prefix
    local matches=()
    if [[ -d "$specs_dir" ]]; then
...
# Resolves FEATURE_DIR: Speckit-style branch → prefix scan; Headway track → .ai/specs/<N-slug>; else fixed_specs_subdir or error.
resolve_feature_dir() {
    local repo_root="$1"
    local branch_name="$2"
    local specs_dir="$repo_root/specs"
    ...
    echo "Set environment SPECIFY_FEATURE to a speckit-style name, or set feature_resolution.fixed_specs_subdir in .ai/specify/init-options.json to your .ai/specs/ folder basename." >&2
    return 1
}
...
resolve_template() {
    ...
    local repo_root="$2"
    local base="$repo_root/.ai/specify/templates"
    ...
    local presets_dir="$repo_root/.ai/specify/presets"
...
" 2>/dev/null); then
    ...
    local ext_dir="$repo_root/.ai/specify/extensions"
    if [ -d "$ext_dir" ]; then
