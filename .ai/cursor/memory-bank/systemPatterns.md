...
Patterns called out in `AGENTS.md` / `client/AGENTS.md` / `server/AGENTS.md` and reinforced in `.ai/cursor/rules/`:
...
- **Compose lifecycle state:** `collectAsStateWithLifecycle()` on a ViewModel `StateFlow` already performs lifecycle-aware collection; do not add no-op `SideEffect` reads of the same state to “keep” the subscription (see `.ai/cursor/rules/lessons-learned.mdc` §20).
...
- **Lessons:** cumulative anti-patterns live in `.ai/cursor/rules/lessons-learned.mdc` (curated, not a dump of every chat).

## Agent workflow layering

- **`AGENTS.md` + `.ai/cursor/rules/`:** canonical coding policy; obey closest `AGENTS.md` by path.
- **`.ai/cursor/memory-bank/`:** canonical durable **project** context (orientation, focus, patterns); maintain via `/finish-feature` and `/capture-lesson` when insights belong outside one-off chat.
- **Spec Kit (`.ai/cursor/commands/speckit.*`, `.ai/specify/`):** feature pipeline — clarify → specify → plan → tasks → implement — with artifacts under `.ai/specs/` (ignored by git here) and `.ai/specify/` templates.
...
- Add feature-specific patterns here only when they are **stable** and **cross-cutting**; otherwise keep them in specs or task notes under `.ai/specify/` / GitBook.
