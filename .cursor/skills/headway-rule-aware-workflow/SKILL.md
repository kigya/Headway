...
description: >-
  Aligns Headway Kotlin edits with .cursor/memory-bank context, the closest
  AGENTS.md, and .cursor/rules (global, client/server, detekt-guardrails,
...
3. Load matching Cursor rules from [.cursor/rules/](../../rules/):
...
6. For recording a mistake after a fix, the user should run `/capture-lesson`. For pre-merge wrap-up, `/finish-feature`. For spec-driven work, use `/speckit.*` commands from [`.cursor/commands/`](../../commands/) per [docs/ai-workflow.md](../../../docs/ai-workflow.md).
